...
int main(){
	...
	srand((unsigned int)time(NULL));
        ...
        createRankList();                               //2주차 실습 구현

	while(!exit){
		clear();
		switch(menu()){
		case MENU_PLAY: play(); break;
                case MENU_RANK: rank(); break;          //2주차 실습 구현
		case MENU_EXIT: exit=1; break;
		default: break;
		}
	}

        writeRankFile();                                //2주차 실습 구현
...
void createRankList(){
	// user code
        ...
        fscanf(fp, "%d ", &cnt);                                        //랭킹정보 개수 입력
        if(cnt != 0)
        {
                newNode = (Node*)malloc(sizeof(Node));
                fscanf(fp, "%s %d ", input_name, &input_score);
...
void rank(){
	// user code
        int x = -1, y =-1, cnt = 0, delRank = 0;
        char command;
        char searchName[NAMELEN];
        Node *currNode = head, *delNode, *tmpNode;
        clear();
        ...
        else if(command == '2')
        {
                ...
                while(currNode != NULL)
                {
                        if(strcmp(currNode->name, searchName) == 0)
                        {
                                printw("%-16s | %-10d\n", currNode->name, currNode->score);
                                refresh();
                                cnt+=1;
        ...
        else if(command == '3')
        {
                ...
                noecho();
                refresh();
                if(delRank <= 0 || delRank > nodeLen || nodeLen == 0)
                {
...
void newRank(int score){
	// user code
        ...
        scanw("%s", player_name);
        noecho();
        strcpy(newNode->name, player_name);
        newNode->score = score;
        if(head == NULL)
        {
                head = newNode;
        ...
        else
        {
                ...
                {
                        if(currNode->link->score > newNode->score)
                                currNode = currNode->link;
                        else break;
                }
